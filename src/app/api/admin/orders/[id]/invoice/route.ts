...
export async function POST(
  ...
  const admin = await requirePermission("invoices", "w");
  if (!admin) {
  ...
  if (order.invoice) {
    return NextResponse.json({ error: "Bu sipariş için zaten fatura kesilmiş" }, { status: 400 });
  }

  if (!["PAID", "PROCESSING", "SHIPPED", "DELIVERED"].includes(order.status)) {
    return NextResponse.json({ error: "Sipariş durumu fatura kesmeye uygun değil" }, { status: 400 });
  }
  ...
  try {
    ...
    const taxAmount = (order.totalAmount * 0.2) / 1.2;
    ...
    await db.orderStatusHistory.create({
      data: {
        ...
        note: `Fatura kesildi: ${result.invoiceNo} (${invoiceType === "EFATURA" ? "E-Fatura" : "E-Arşiv"})`,
